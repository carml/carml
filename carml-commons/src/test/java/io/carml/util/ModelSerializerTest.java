...
class ModelSerializerTest {
  ...
  void givenModel_whenSerializeAsRdf_thenReturnExpectedSerialization() {
    // Given
    var model = new ModelBuilder().subject("http://example.org/subject")
  ...
  void givenModelAndNamespaceApplier_whenSerializeAsRdf_thenReturnPrefixedSerialization() {
    ...
    // Then
    assertThat(result, equalToCompressingWhiteSpace("@prefix owl: <http://www.w3.org/2002/07/owl#> ." //
  ...
  void given_whenFormatResourceForLog_then() {
    ...
    var model = new ModelBuilder().subject(subject)
        .add(RDF.TYPE, OWL.CLASS)
        .add(RDFS.LABEL, "foo")
        .build();
  ...
  void givenBlankNodeResource_whenFormatResourceForLog_then() {
    ...
    // Then
    assertThat(logString, equalToCompressingWhiteSpace("blank node resource _:bar in: " //
        + "```" //
