...
public class DuckDbLogicalViewEvaluator implements LogicalViewEvaluator {
    ...
    public Flux<ViewIteration> evaluate(
        ...
        // Defer compilation so that errors from DuckDbViewCompiler.compile() are emitted as
        ...
        return PausableFluxBridge.<ViewIteration>builder()
                .sourceFactory(emitter -> {
                    var source = new DuckDbPausableSource(self -> {
                        ...
                        try {
                            ...
                            var compiledView =
                                    DuckDbViewCompiler.compile(view, context, sourceTableResolver, databaseAttacher);
                            LOG.debug("Executing DuckDB query for view [{}]", view.getResourceName());
                            executeQuery(emitter, self, compiledView, view, context);
                            self.complete();
                            emitter.complete();
                        } finally {
                            // Clean up on the producer thread to avoid racing with onDispose.
                            // The producer thread owns the JDBC resources (ResultSet, Statement,
                            // Connection) — closing them here ensures executeQuery's
                            // try-with-resources has completed before the connection is closed.
    ...
    private void executeQuery(
            PausableFluxBridge.Emitter<ViewIteration> emitter,
        ...
        try (var statement = connection.createStatement();
            ...
            LOG.debug("DuckDB query failed for view [{}]: {}", view.getResourceName(), compiledView.sql());
    ...
    private void emitRows(
        ...
        while (resultSet.next()) {
            ...
            var zeroBasedIndex = readZeroBasedIndex(resultSet, columns);
            var values = readColumnValues(resultSet, columns, zeroBasedIndex);

            validateNoNonScalarTypes(resultSet, columns, compiledView);

            var naturalDatatypes = resolveNaturalDatatypes(resultSet, columns, values);
            var sourceEvaluation = resolveSourceEvaluation(resultSet, columns, retainSourceEvaluation);
    ...
    private static int readZeroBasedIndex(ResultSet resultSet, ColumnDescriptor columns) throws SQLException {
    ...
    private static LinkedHashMap<String, Object> readColumnValues(
            ResultSet resultSet, ColumnDescriptor columns, int zeroBasedIndex) throws SQLException {
        ...
        for (var colName : columns.valueNames) {
            values.put(colName, readColumnValue(resultSet, colName, columns));
        }
        return values;
    }

    private static Object readColumnValue(ResultSet resultSet, String colName, ColumnDescriptor columns)
            throws SQLException {
     ...
     * requests it (e.g., implicit views with gather expressions), matching the reactive evaluator's
     * behavior. Without this guard, the source fallback in RdfTriplesMapper would silently swallow
     * invalid field references.
     */
    private static ExpressionEvaluation resolveSourceEvaluation(
            ResultSet resultSet, ColumnDescriptor columns, boolean retainSourceEvaluation) throws SQLException {
     ...
     *
     * <p>The set of non-scalar type values is determined by the source strategy during compilation
     * and bundled into the {@link CompiledView}. For JSON sources, these are {@code "ARRAY"} and
     * {@code "OBJECT"}. For column-based sources (CSV, SQL), the set is empty, so this validation
     * is effectively a no-op.
     *
     * @param compiledView the compiled view containing multi-valued field names and non-scalar type
     *     values
    ...
    private static void validateNoNonScalarTypes(
            ResultSet resultSet, ColumnDescriptor columns, CompiledView compiledView) throws SQLException {
