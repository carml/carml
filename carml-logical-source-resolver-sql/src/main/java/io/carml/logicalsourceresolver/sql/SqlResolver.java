...
public abstract class SqlResolver implements LogicalSourceResolver<RowData> {
  ...
  private Flux<LogicalSourceRecord<RowData>> getLogicalSourceRecordFlux(ResolvedSource<?> resolvedSource,
      Set<LogicalSource> logicalSources) {
  ...
  private Flux<LogicalSourceRecord<RowData>> getResultFlux(ConnectionFactory connectionFactory,
      Set<LogicalSource> logicalSources) {
    var logicalSourcesGroupedByQuery = logicalSources.stream()
        .collect(groupingBy(this::getSelectQueryString, toUnmodifiableSet()));
    ...
    return Flux.fromIterable(logicalSourcesGroupedByQuery.entrySet())
        .concatMap(queryEntry -> getResultFluxForQueryString(
            /* getPooledConnectionFactory( */connectionFactory/* , logicalSources.size()) */, queryEntry.getKey(),
            queryEntry.getValue()));
  }
  ...
  private Flux<LogicalSourceRecord<RowData>> getResultFluxForQueryString(ConnectionFactory connectionFactory,
      String selectQuery, Set<LogicalSource> logicalSources) {
    ...
    return Flux.fromIterable(logicalSources)
        .concatMap(logicalSource -> Mono.from(connectionFactory.create())
            .flatMapMany(connection -> {
              var statement = connection.createStatement(selectQuery);
  ...
  public static String getQuery(SQLDialect sqlDialect, LogicalSource logicalSource) {
    var fields = logicalSource.getExpressions()
        .stream()
  ...
  public static String getJointSqlQuery(SQLDialect sqlDialect, JoiningDatabaseSource joiningDatabaseSourceSupplier) {
    ...
    var childTable = childSelect.asTable()
        .as(CHILD_ALIAS);
    ...
    var parentTable = parentSelect.asTable()
        .as(PARENT_ALIAS);
  ...
  private static SelectQuery<?> getSelect(DSLContext ctx, LogicalSource logicalSource, Set<Field<Object>> projection) {
    if (logicalSource.getQuery() != null) {
      return (SelectQuery<?>) ctx.parser()
          .parseSelect(logicalSource.getQuery());
    } else {
      return ctx.select(projection)
