...
public class RdfExpressionMapEvaluation {
  ...
  private List<Object> evaluateReference() {
    return expressionEvaluation.apply(expressionMap.getReference())
  ...
  private void bindTemplateExpression(ReferenceExpression expression,
      TemplateEvaluationBuilder templateEvaluatorBuilder) {
    var datatype = datatypeMapper == null ? XSD.STRING
        : datatypeMapper.apply(expression.getValue())
            .orElse(XSD.STRING);
  ...
  private String transformValueForTemplate(Object result, IRI datatype) {
    return rdfLexicalForm.andThen(templateReferenceValueTransformingFunction)
