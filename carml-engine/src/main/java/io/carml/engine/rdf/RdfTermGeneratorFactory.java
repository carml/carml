...
public class RdfTermGeneratorFactory implements TermGeneratorFactory<Value> {
  ...
  private void validateTermType(TermMap termMap, Set<TermType> allowedTermTypes) {
    if (termMap.getTermType() != null && !allowedTermTypes.contains(termMap.getTermType())) {
      throw new TermGeneratorFactoryException(
          String.format("encountered term type %s, which is not allowed for term map%n%s", termMap.getTermType(),
              exception(termMap)));
    }
  }
  ...
  public TermGenerator<Value> getObjectGenerator(ObjectMap objectMap) {
    ...
    return (TermGenerator<Value>) createTermGenerator(objectMap, objectMap.getTermType(),
        Set.of(IRI.class, Literal.class), generateOverrideDatatypes, objectMap.getLanguageMap());
  }
  ...
  private TermGenerator<? extends Value> createTermGenerator(ExpressionMap expressionMap, TermType termType,
  ...
  private TermGenerator<? extends Value> createTermGenerator(ExpressionMap expressionMap, TermType termType,
      Set<Class<? extends Value>> allowedConstantTypes, TermGenerator<IRI> generateOverrideDatatypes,
  ...
  private TermGenerator<? extends Value> getConstantGenerator(ExpressionMap expressionMap,
      Set<Class<? extends Value>> allowedConstantTypes) {
    Value constant = expressionMap.getConstant();
    if (allowedConstantTypes.stream()
        .noneMatch(allowed -> allowed.isInstance(constant))) {
      throw new TermGeneratorFactoryException(String
          .format("encountered constant value of type %s, which is not allowed for this term map", constant.getClass()
              .getSimpleName()));
    }
  ...
  private TermGenerator<? extends Value> getReferenceGenerator(ExpressionMap expressionMap, TermType termType,
      TermGenerator<IRI> generateOverrideDatatypes, LanguageMap languageMap) {
    ...
    return (expressionEvaluation, datatypeMapper) -> {
      ...
      var mappedDatatype = datatypeMapper == null ? null
          : datatypeMapper.apply(reference)
              .orElse(null);
      var overrideDatatypes = getOverrideDatatypes(generateOverrideDatatypes, expressionEvaluation, datatypeMapper);
  ...
  private List<String> getLanguageTags(LanguageMap languageMap, ExpressionEvaluation expressionEvaluation,
  ...
  private TermGenerator<? extends Value> getTemplateGenerator(ExpressionMap expressionMap, TermType termType,
      TermGenerator<IRI> generateOverrideDatatypes, LanguageMap languageMap) {
    UnaryOperator<String> valueTransformingFunction = termType == TermType.IRI ? makeIriSafe : UnaryOperator.identity();
  ...
  private TermGenerator<? extends Value> getFunctionValueGenerator(ExpressionMap expressionMap, TermType termType,
      TermGenerator<IRI> generateOverrideDatatypes, LanguageMap languageMap) {
    return (expressionEvaluation, datatypeMapper) -> {
  ...
  private List<IRI> getOverrideDatatypes(TermGenerator<IRI> generateOverrideDatatypes,
      ExpressionEvaluation expressionEvaluation, DatatypeMapper datatypeMapper) {
    return generateOverrideDatatypes != null ? generateOverrideDatatypes.apply(expressionEvaluation, datatypeMapper)
        : List.of();
  }

  private List<Value> generateTerms(List<Object> values, TermType termType, IRI mappedDatatype,
      List<IRI> overrideDatatypes, List<String> languageTags) {
    return switch (termType) {
      case IRI -> values.stream()
          .map(value -> CanonicalRdfLexicalForm.get()
              .apply(value, mappedDatatype))
  ...
  private List<Value> generateLiteralTerms(List<Object> values, IRI mappedDatatype, List<IRI> overrideDatatypes,
