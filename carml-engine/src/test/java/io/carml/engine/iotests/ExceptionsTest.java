...
class ExceptionsTest extends MappingTester {
    ...
    void testUnknownTermTypeException() {
        RuntimeException exception = assertThrows(
                RuntimeException.class,
                () -> testMapping(
                        "RmlMapper", "/RmlMapper/exceptionTests/exceptionUnknownTermTypeMapping.rml.ttl", null));
        assertThat(
                exception.getMessage(),
                startsWith(String.format(
                        "cannot create an instance of enum type [%s]", TermType.class.getCanonicalName())));
    }
    ...
    void testConstantValueException() {
        RuntimeException exception = assertThrows(
                RuntimeException.class,
                () -> testMapping(
                        "RmlMapper", "/RmlMapper/exceptionTests/exceptionConstantValueMapping.rml.ttl", null));
    ...
    void testEqualLogicalSourceException() {
        RuntimeException exception = assertThrows(
                RuntimeException.class,
                () -> testMapping(
                        "RmlMapper", "/RmlMapper/exceptionTests/exceptionEqualLogicalSourceMapping.rml.ttl", null));
    ...
    void testTermTypeExceptionB() {
        ...
        assertThat(
                exception.getMessage(), startsWith("encountered term type LITERAL, which is not allowed for term map"));
    }
