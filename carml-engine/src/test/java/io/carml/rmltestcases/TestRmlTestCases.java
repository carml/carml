...
public class TestRmlTestCases {
  ...
  private void prepareForDatabaseTest(TestCase testCase, JdbcDatabaseContainer<?> container,
      Function<JdbcDatabaseContainer<?>, ConnectionFactoryOptions> optionsGetter) {
    testCase.getInput()
        .stream()
        .map(TestRmlTestCases::getInputInputStream)
        .forEach(inputStream -> {
          try (Connection conn =
  ...
  private Model executeMapping(TestCase testCase) {
    ...
    if (testCase.getId()
      ...
      prepareForDatabaseTest(testCase, mysql,
    ...
    if (testCase.getId()
      ...
      prepareForDatabaseTest(testCase, postgresql,
  ...
  static InputStream getInputInputStream(Input input) {
    String relativeLocation = input.getDistribution()
        .getRelativeFileLocation();
    return TestRmlTestCases.class.getResourceAsStream(relativeLocation);
  }
}
