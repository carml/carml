...
public class RmlImplementationReport {
                            ...
                            try {
                                ...
                                if (error != null) {
                                    errorPrinter.printRecord(
                                            result.getTestCase().getIdentifier(), error);
                                }
                            } catch (IOException e) {
    ...
    static Function<Model, Set<Resource>> selectTestCases = model -> Set.copyOf(
            model.filter(null, RDF.TYPE, TestRmlTestCases.EARL_TESTCASE).subjects());
    ...
    public static Set<TestCase> populateTestCases() {
        ...
        return RdfObjectLoader.load(
                        selectTestCases,
                        RmlTestCaze.class,
                        Models.parse(Objects.requireNonNull(metadata), RDFFormat.NTRIPLES))
                .stream()
    ...
    public static TestCaseResult runTestCase(TestCase testCase) {
        ...
        TestCaseResult.Builder resultBuilder = TestCaseResult.builder().testCase(testCase);

        if (!isSupported(testCase.getId())) {
            return resultBuilder.testResultInapplicable().build();
        }
        ...
        return resultBuilder.testResult(passed).build();
    }
