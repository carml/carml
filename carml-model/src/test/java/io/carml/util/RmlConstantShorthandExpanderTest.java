...
class RmlConstantShorthandExpanderTest {
  ...
  void expander_givenShorthandWithContext_ShouldHaveContextOnExpandedStatements() {
    ...
    int nrOfstatementsWithContext = (int) expanded.stream()
        .filter(st -> st.getContext() != null)
        .count();

    assertThat(nrOfstatementsWithContext, is(2));

    assertThat(expanded.contexts()
        .size(), is(1));
  ...
  private Model expandAndTestShorthandStatement(Statement toExpand, IRI expandedPredicate) {
    ...
    Statement firstStatement = Iterators.getOnlyElement(expanded.filter(null, expandedPredicate, null)
        .iterator());
    assertThat(firstStatement.getSubject(), is(toExpand.getSubject()));
    assertThat(firstStatement.getObject(), instanceOf(BNode.class));

    Statement otherSt = Iterators.getOnlyElement(expanded.filter((Resource) firstStatement.getObject(), null, null)
        .iterator());
