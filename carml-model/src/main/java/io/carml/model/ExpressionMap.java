...
public interface ExpressionMap extends Resource {
  ...
  default Set<String> getExpressionMapExpressionSet() {
    ...
    if (getTemplate() != null) {
      return getTemplate().getReferenceExpressions()
          .stream()
    ...
    if (getFunctionValue() != null) {
      return getFunctionValue().getPredicateObjectMaps()
          .stream()
          .flatMap(predicateObjectMap -> Stream.concat(predicateObjectMap.getPredicateMaps()
              .stream()
              .flatMap(predicateMap -> predicateMap.getExpressionMapExpressionSet()
                  .stream()),
              predicateObjectMap.getObjectMaps()
                  .stream()
                  .filter(ObjectMap.class::isInstance)
                  .map(ObjectMap.class::cast)
                  .flatMap(objectMap -> objectMap.getExpressionMapExpressionSet()
                      .stream())))
          .collect(Collectors.toUnmodifiableSet());
    }
