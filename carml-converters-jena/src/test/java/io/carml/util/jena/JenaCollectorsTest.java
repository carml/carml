...
class JenaCollectorsTest {
    ...
    void givenStatements_whenToDatasetGraph_thenReturnsDatasetGraph() {
        ...
        datasetGraph
                .listGraphNodes()
                .forEachRemaining(node -> assertThat(
                        node.getURI(),
                        in(List.of(
                                "http://example.org/graph1", "http://example.org/graph2", defaultGraphIRI.getURI()))));

        var graph1 = datasetGraph.getGraph(createURI("http://example.org/graph1"));
        assertThat(graph1.stream().count(), is(2L));
        assertThat(
                graph1.contains(
                        createURI("http://example.org/subject1"),
                        createURI(RDF.TYPE.stringValue()),
                        createURI(RDFS.RESOURCE.stringValue())),
                is(true));
        assertThat(
                graph1.contains(
                        createURI("http://example.org/subject1"),
        ...
        assertThat(graph2.stream().count(), is(2L));
        assertThat(
                graph2.contains(
        ...
        assertThat(defaultGraph.stream().count(), is(2L));
        assertThat(
    ...
    void givenStatements_whenToDataset_thenReturnsDataset() {
        ...
        assertThat(
                model1.contains(
                        createResource("http://example.org/subject1"),
                        createProperty(RDF.TYPE.stringValue()),
                        createResource(RDFS.RESOURCE.stringValue())),
                is(true));
        assertThat(
                model1.contains(
                        createResource("http://example.org/subject1"),
                        createProperty(RDFS.LABEL.stringValue()),
                        createPlainLiteral("subject1")),
                is(true));
        ...
        assertThat(
                model2.contains(
                        createResource("http://example.org/subject2"),
                        createProperty(RDF.TYPE.stringValue()),
                        createResource(RDFS.RESOURCE.stringValue())),
                is(true));
        assertThat(
        ...
        assertThat(
                defaultModel.contains(
                        createResource("http://example.org/subject"),
                        createProperty(RDF.TYPE.stringValue()),
                        createResource(RDFS.RESOURCE.stringValue())),
                is(true));
        assertThat(
