...
class JenaConvertersTest {
  ...
  static Stream<Arguments> toNodeTestArguments() {
    return Stream.of(//
        ...
        Arguments.of(literal("bar", "en"), (Predicate<Node>) Node::isLiteral,
            Map.of((Function<Node, String>) Node::getLiteralLexicalForm, "bar", Node::getLiteralLanguage, "en")),
        Arguments.of(literal("baz", iri("http://example.org/")), (Predicate<Node>) Node::isLiteral,
            Map.of((Function<Node, String>) Node::getLiteralLexicalForm, "baz", Node::getLiteralDatatypeURI,
                "http://example.org/")),
        Arguments.of(triple(iri("http://example.org/resource"), RDFS.LABEL, literal("bar", "nl")),
            (Predicate<Node>) Node::isNodeTriple, Map.of(//
                (Function<Node, String>) node -> node.getTriple()
                    .getSubject()
                    .getURI(),
  ...
  void givenValue_whenToNode_thenReturnsExpectedNode(Value value, Predicate<Node> nodeTypeChecker,
      Map<Function<Node, Object>, Object> valueCheckers) {
    // Given
    // When
  ...
  void givenStatement_whenToQuad_thenReturnsQuad() {
    // Given
    var statement = Statements.statement(iri("http://example.org/resource"), RDFS.LABEL, literal("bar", "nl"),
    ...
    // Then
    assertThat(quad.getSubject()
        .getURI(), is("http://example.org/resource"));

    assertThat(quad.getPredicate()
        .getURI(), is("http://www.w3.org/2000/01/rdf-schema#label"));

    assertThat(quad.getObject()
        .getLiteralLexicalForm(), is("bar"));

    assertThat(quad.getObject()
        .getLiteralLanguage(), is("nl"));

    assertThat(quad.getGraph()
        .getURI(), is("http://example.org/graph"));
  }
}
