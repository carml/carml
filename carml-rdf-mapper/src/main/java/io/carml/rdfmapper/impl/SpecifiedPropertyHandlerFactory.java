...
public class SpecifiedPropertyHandlerFactory {
    ...
    private <T extends PropertyHandler> T createInstance(Class<T> handlerCls) {
        try {
            ...
            throw new CarmlMapperException(
                    String.format(
                            "could not instantiate specified PropertyHandler class [%s]",
                            handlerCls.getCanonicalName()),
                    exception);
        }
    }
    ...
    private void injectDependencies(Class<?> cls, DependencyResolver resolver, Object instance) {
        cache.get(cls).orElse(createAndCacheDependencySetters(cls, resolver)).forEach(i -> i.accept(instance));
    }
    ...
    private Type getPropertyType(Method method) {
        ...
        if (parameterTypes.size() != 1) {
            throw new CarmlMapperException(String.format(
                    "method [%s], annotated with @Inject does NOT take exactly 1 parameter; it takes %s",
    ...
    private List<Annotation> getPropertyQualifiers(Method method) {
        return stream(method.getAnnotations()).filter(this::isQualifierInstance).toList();
    }

    private Consumer<Object> createDependencySetter(
            Method method, DependencyResolver resolver, Type propertyType, List<Annotation> qualifiers) {
        return i -> {
            ...
            try {
                ...
                throw new CarmlMapperException(
                        String.format("error invoking setter [%s]", method.getName()), exception);
            }
        };
    }
}
