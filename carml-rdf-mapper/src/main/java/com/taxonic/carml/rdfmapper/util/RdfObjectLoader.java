...
public class RdfObjectLoader {
  ...
  private static final String RESOURCE_SELECTOR_MSG = "A resource selector must be provided",
      CLASS_MSG = "A class must be provided", MODEL_MSG = "A model must be provided",
      MODEL_ADAPTER_MSG = "A model adapter must be provided", REPOSITORY_MSG = "A repository must be provided",
      SPARQL_QUERY_MSG = "A SPARQL query must be provided", POPULATE_CACHE_MSG = "A cache populator must be provided";
  ...
  public static <T> Set<T> load(Function<Model, Set<Resource>> resourceSelector, Class<T> clazz, Model model,
      UnaryOperator<Model> modelAdapter, Consumer<MappingCache> populateCache, Consumer<Mapper> configureMapper) {
  ...
  public static <T> Set<T> load(Function<Model, Set<Resource>> resourceSelector, Class<T> clazz, Repository repository,
      String sparqlQuery) {
  ...
  public static <T> Set<T> load(Function<Model, Set<Resource>> resourceSelector, Class<T> clazz, Repository repository,
      String sparqlQuery, UnaryOperator<Model> modelAdapter) {
  ...
  public static <T> Set<T> load(Function<Model, Set<Resource>> resourceSelector, Class<T> clazz, Repository repository,
      UnaryOperator<Model> modelAdapter, Resource... contexts) {
